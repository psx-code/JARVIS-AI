...
# --- away mode / autoresponder ---
from actions.notification_watcher import NotificationWatcher
from actions.auto_reply           import AutoReplyEngine
...
def _prevent_sleep():
    ...
    print("[JARVIS] 💡 Sleep prevention ON — system will stay awake.")


def _allow_sleep():
    """Restore normal sleep behavior on shutdown."""
    ctypes.windll.kernel32.SetThreadExecutionState(0x80000000)
    print("[JARVIS] 💤 Sleep prevention OFF.")
...
TOOL_DECLARATIONS = [
    ...
    },
    {
        "name": "set_away_mode",
        "description": (
            "Turns JARVIS's away-mode autoresponder on or off. When ON, JARVIS watches "
            "WhatsApp and replies on the user's behalf, escalating urgent messages. "
            "Call this whenever the user says they're going away, sleeping, or in a meeting. "
            "Call again with active=false when they return — JARVIS will summarize what happened."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "active":     {"type": "BOOLEAN", "description": "true to turn on, false to turn off and get briefing"},
                "reason":     {"type": "STRING",  "description": "Why the user is away"},
                "whitelist":  {"type": "ARRAY", "items": {"type": "STRING"}, "description": "Only auto-reply to these contacts"},
                "blacklist":  {"type": "ARRAY", "items": {"type": "STRING"}, "description": "Never auto-reply to these contacts"},
                "sign_reply": {"type": "BOOLEAN", "description": "Append '(auto-reply)' to messages"},
            },
            "required": ["active"]
        }
    },
...
class JarvisLive:
    ...
    def __init__(self, ui: JarvisUI):
        ...
        # away-mode / notification autoresponder
        ...
        try:
            if name == "open_app":
                ...
                if args.get("active", True):
                    self.auto_reply.go_away(
                        reason=args.get("reason", ""),
                        whitelist=args.get("whitelist"),
                        blacklist=args.get("blacklist"),
                        sign_reply=bool(args.get("sign_reply", False)),
                    )
                    result = f"Away mode activated. {args.get('reason', '')}".strip()
                else:
                    result = self.auto_reply.come_back()
    ...
    async def _notification_watcher_task(self):
        try:
            watcher = NotificationWatcher(
                on_notification=lambda notif: threading.Thread(
                    target=self.auto_reply.handle_notification,
                    args=(notif,),
                    daemon=True
                ).start()
            )
        except RuntimeError as e:
            print(f"[JARVIS] ⚠️ Notification watcher disabled: {e}")
            return
        try:
            await watcher.start()
        except Exception:
        ...
        while True:
            try:
                ...
                async with (
                    ...
                    tg.create_task(self._notification_watcher_task())
...
def main():
    _prevent_sleep()   # keep system awake while JARVIS runs
